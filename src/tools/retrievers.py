...
def _format_pinecone_results(matches: List[dict]) -> List[str]:
    ...
    for match in matches:
        ...
        if page_numbers_raw:
            ...
            if len(unique_pages) > MAX_PAGES_TO_SHOW: page_str = ", ".join(map(str, unique_pages[:MAX_PAGES_TO_SHOW])) + ", ..."
            else: page_str = ", ".join(map(str, unique_pages))
            if unique_pages: link_url = f"{url}#page={unique_pages[0]}"
...
# --- START OF THE ABSOLUTE FINAL, DEFINITIVE NEO4J SERIALIZER ---
def _serialize_neo4j_path(record: Dict[str, Any]) -> str:
    ...
    try:
        ...
        # --- THIS IS THE NEW LOGIC TO FIX THE BUG ---
            ...
            # Handle the format seen in the logs: [dict, str, dict]
            if isinstance(path_data[0], dict) and isinstance(path_data[1], str) and isinstance(path_data[2], dict):
                subject_name = path_data[0].get('name')
                predicate_type = path_data[1] # The relationship is just a string
        ...
        if not all([subject_name, predicate_type, object_name]):
            logger.warning(f"Could not fully parse Neo4j path data: {path_data}")
            return ""
        ...
        if isinstance(rel_props, dict):
            doc_id = rel_props.get('doc_id'); url = rel_props.get('source_pdf_url'); page_num_str = rel_props.get('page_numbers', 'N/A')
            if doc_id: citation_text = f"{doc_id} (Page {page_num_str})"
            if url:
                first_page = page_num_str.split(',')[0].strip()
                if first_page.isdigit(): link_url = f"{url}#page={first_page}"
                else: link_url = url
        ...
        return ""
# --- END OF THE ABSOLUTE FINAL, DEFINITIVE NEO4J SERIALIZER ---
...
def _vector_search_tool(query: str, namespace: str, tool_name: str, top_k: int = 7) -> ToolResult:
    ...
    with Timer(f"Tool: {tool_name}"):
        ...
        if not pinecone_index or not genai: return ToolResult(tool_name=tool_name, success=False, content="Vector search client not available.")
        try:
